...
#         )
#         description = self.extract_element_text("#productDescription")
...
class Scraper:
    ...
    def get_driver(self):
        ...
        try:
            print("Launching browser with dynamic ChromeDriver...")
            ...
            print("❌ SessionNotCreatedException. Make sure Chrome is up to date.")
            raise e
        except WebDriverException as e:
            print("❌ WebDriverException occurred. Check Chrome installation.")
            raise e
    ...
    def get(self):
        ...
        if "captcha" in driver.current_url.lower():
            print("⚠️ CAPTCHA detected! Try rotating IPs.")
            driver.quit()
    ...
    def scrape(self):
        ...
        customer_reviews = html.find("#acrCustomerReviewText", first=True)
        if customer_reviews:
            match = re.search(r'([\d,]+)', customer_reviews.text)
            if match:
                total_reviews = int(match.group(1).replace(',', ''))
        ...
        tables = self.extract_tables()
        specs = self.extract_table_dataset(tables)
        price = specs.get("price_raw", price)
        total = specs.get("total_raw")
        total_str = clean_price(total)
        price_str = clean_price(price)
